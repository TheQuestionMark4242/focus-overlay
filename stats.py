"""Statistics window: pie chart of time-per-task and a timeline of work sessions."""
...
class StatsWindow:
    def __init__(self, master: tk.Tk):
        ...
        self.pie_ax = self.figure.add_subplot(1, 2, 1)
        self.timeline_ax = self.figure.add_subplot(1, 2, 2)
    ...
    def _draw_timeline(self, ax, rows) -> None:
        ...
        now = datetime.now()
        for row in rows:
            y_index = titles.index(row["title"])
            start = datetime.fromisoformat(row["start_time"])
            end = datetime.fromisoformat(row["end_time"]) if row["end_time"] else now
            start_num = mdates.date2num(start)
            width_num = mdates.date2num(end) - start_num
            ax.broken_barh([(start_num, width_num)], (y_index * 10, 9), facecolors="tab:blue")

        ax.set_yticks([i * 10 + 4.5 for i in range(len(titles))])
